# MILESTONE-TRACKER-v4.md
# VStocker 里程碑與任務追蹤

**最後更新：** 2026-06-18  
**關聯計畫：** [PLAN-VStocker-v4.md](./PLAN-VStocker-v4.md)
...
| M1.5 k8s 前置部署 | 2026-08-23 | 未開始 | 0% |
| M2 台股 + Switch + 版塊 + 儀表板 | 2026-10-04 | 未開始 | 0% |
| M3 AI 個股分析 | 2026-11-15 | 未開始 | 0% |
...
## Phase 2：k8s 移植 → 台股 + Switch + 版塊 + 儀表板

### P2-K8S：Kubernetes 基礎設施（M1 完成後立即進行）

> Docker Compose 保留本機開發用，k8s 為 Server 部署環境

| 任務 ID | 任務描述 | 狀態 | 完成日 | 備註 |
|---|---|---|---|---|
| P2-K8S-01 | k8s 叢集選型與建立（GKE/EKS/AKS 或 k3s） | 待辦 | - | 先決策雲端 vs 自建 |
| P2-K8S-02 | Container Registry 設定（GHCR） | 待辦 | - | |
| P2-K8S-03 | 各服務 Production-grade Dockerfile（multi-stage build） | 待辦 | - | |
| P2-K8S-04 | Helm Chart 撰寫（5 個服務 + gateway） | 待辦 | - | 最耗時任務 |
| P2-K8S-05 | Nginx Ingress Controller 設定 | 待辦 | - | |
| P2-K8S-06 | k8s Secret 設定（JWT Secret, DB 密碼, API Key） | 待辦 | - | |
| P2-K8S-07 | Liveness / Readiness Probe 設定（Spring Actuator） | 待辦 | - | |
| P2-K8S-08 | PostgreSQL 部署策略（PVC StatefulSet 或 Managed DB） | 待辦 | - | 優先考慮 Managed DB |
| P2-K8S-09 | Redis 部署（StatefulSet 或 Managed Redis） | 待辦 | - | |
| P2-K8S-10 | CI/CD Pipeline（GitHub Actions: build → push → helm upgrade） | 待辦 | - | |
| P2-K8S-11 | Phase 1 服務全部部署至 k8s 並驗證通過 | 待辦 | - | k8s 前置里程碑 |
...
| 任務 ID | 任務描述 | 狀態 | 完成日 | 備註 |
|---|---|---|---|---|
| P2-Port-01 | 台股持倉 CRUD（幣別 TWD） | 待辦 | - | |
| P2-Port-02 | 美股損益 USD→TWD 換算 | 待辦 | - | P1 優先級 |
...
| P3-AI-05 | ai-service Helm Chart + k8s 部署（含 Resource Limit） | 待辦 | - | |
...
| P3-X-06 | k8s HPA 設定（market-data, ai-service） | 待辦 | - | P1 優先級 |
...
| 項目 | 選項 | 狀態 |
|---|---|---|
| k8s 叢集 | GKE / EKS / AKS / k3s | 待決策 |
| 資料庫 | PVC StatefulSet / Managed DB | 待決策（優先 Managed DB） |
| Redis | StatefulSet / Managed Redis | 待決策 |
| Container Registry | GHCR（預設） | 待確認 |
...
| 2026-06-18 | v4.0 | 新增 M1.5 k8s 前置 sprint（11 個任務），M2 日期調整至 2026-10-04，M3 至 2026-11-15 |
